    ...
    <section class="content">
        ...
        <div class="box">
            <div class="box-header with-border">
                <h3 class="box-title">List Student
                                ...
                                @if($data['students'] == $data['students']->count() > 0 )
                                    ...
                                    @foreach($data['students'] as $student)
                                        <tr role="row" class="odd">
                                            ...
                                            <td class="sorting_1">{{ $student->permanent_address }}</td>
                                            <td class="sorting_1">{{ $student->parent_name }}</td>
                                            @if ($student->photo)
                                                <td><img src="{{asset('images/student').'/'.$student->photo }}" height="70" width="70" alt="{{$student->photo}}"/></td>
                                            @else
                                                <td>No Image</td>
                                            @endif
                                            <td>{{ date('j M Y', strtotime($student->created_at)) }} </td>
                                            <td>
                                                <div class="bedit">
{{--                                                    <a class="btn btn-primary"--}}
{{--                                                       href="{{ route('student.followup', ['id' => $student->id]) }}" title="Follow Up"><i--}}
{{--                                                            class="glyphicon glyphicon-calendar"></i></a>--}}
                                                    <a class="btn btn-success"
                                                       href="{{ route('student.show', ['id' => $student->id]) }}" title="View Details"><i
                                                            class="glyphicon glyphicon-eye-open"></i></a>
                                                    <a class="btn btn-warning"
                                                       href="{{ route('student.edit', ['id' => $student->id]) }}" title="Edit"><i
                                                            class="glyphicon glyphicon-edit"></i></a>

                                                    {!! Form::open(['route' => ['student.destroy', $student->id], 'data-id'=> $student->id, 'class' => 'registration-delete', 'onsubmit' => 'handleTypeDelete(event)', 'method' => 'delete']) !!}
