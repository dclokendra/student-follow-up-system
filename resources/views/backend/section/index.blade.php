    ...
    <section class="content">
        ...
        <div class="box">
            <div class="box-header with-border">
                <h3 class="box-title">List Section
                    <a href="{{route('section.create')}}" class="btn btn-info"><i class="fa fa-plus"></i>Create</a>
                </h3>
                                ...
                                @if($data['sections'] == $data['sections']->count() > 0 )
                                    ...
                                    @foreach($data['sections'] as $section)
                                        <tr role="row" class="odd">
                                            ...
                                            <td>{{ date('j M Y', strtotime($section->created_at)) }} </td>
                                            <td>
                                                <div class="bedit">
                                                    <a class="btn btn-success"
                                                       href="{{ route('section.show', ['id' => $section->id]) }}" title="View Details"><i
                                                            class="glyphicon glyphicon-eye-open"></i></a>
                                                    <a class="btn btn-warning"
                                                       href="{{ route('section.edit', ['id' => $section->id]) }}" title="Edit"><i
                                                            class="glyphicon glyphicon-edit"></i></a>

                                                    {!! Form::open(['route' => ['section.destroy', $section->id], 'data-id'=> $section->id, 'class' => 'registration-delete', 'onsubmit' => 'handleTypeDelete(event)', 'method' => 'delete']) !!}
